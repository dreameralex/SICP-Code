...
function prob(name, wire){
    add_action(wire,
        ()=>BasicTool.display(name + " " + 
                    BasicTool.stringfy(current_time(the_agenda)) + ", new value = " + 
                    BasicTool.stringfy(get_signal(wire))
        )
    )
}
...
BasicTool.display("A Sample simulation End")


// Implemnetin the agenda
...
function make_agenda(){return BasicTool.list(0)}

function print_agenda(agenda){
    BasicTool.print_list(agenda)
}

function current_time(agenda){ return BasicTool.head(agenda)}

function set_current_time(agenda, time){
    BasicTool.set_head(agenda, time)
}

function segments(agenda) {return BasicTool.tail(agenda)} 

function set_segments(agenda, segs){
    BasicTool.set_tail(agenda, segs)
}

function first_segment(agenda){return BasicTool.head(segments(agenda))}

function rest_segments(agenda){return BasicTool.tail(segments(agenda))}
...
function add_to_agenda(time, action, agenda){
    ...
    function add_to_segments(segs){
        ...
        if(segment_time(BasicTool.head(segs)) === time){
            ...
            const rest = BasicTool.tail(segs);
            // console.log("add to segments2")
            if(belongs_before(rest)){
                BasicTool.set_tail(segs,
                    BasicTool.pair(make_new_time_segment(time, action),
                                    BasicTool.tail(segs)))
            }else{
    ...
    if(belongs_before(segs)){
        ...
        set_segments(agenda,
            BasicTool.pair(make_new_time_segment(time, action), segs))
    }else{
...
function first_agenda_item(agenda){
    if(is_empty_agenda(agenda)){
        ...
        // console.log("first_agenda_item:",first_seg)
        set_current_time(agenda, segment_time(first_seg));
        // console.log("first_agenda_item:",segment_queue(first_seg),BasicTool.head(segment_queue(first_seg)))
...
agenda = make_agenda()
print_agenda(agenda)
console.log("Is the agenda empty? ", is_empty_agenda(agenda))
...
print_agenda(agenda)
console.log('current time',current_time(agenda))
// Add item
